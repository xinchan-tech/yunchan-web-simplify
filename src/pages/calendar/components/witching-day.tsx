...
export const WitchingDay = () => {
  ...
  const columns = useMemo<JknTableProps<any>['columns']>(
    () => [
      {
        header: () => <span className="text-stock-up">一季度</span>,
        accessorKey: '0',
        enableSorting: false,
        meta: { align: 'center' },
        cell: ({ row }) => (
          <span className="text-white inline-block my-2">
            {row.original[0]} ({dateToWeek(row.original[0], '周')})
          </span>
        )
      },
      {
        header: () => <span className="text-stock-up">二季度</span>,
        accessorKey: '1',
        enableSorting: false,
        meta: { align: 'center' },
        cell: ({ row }) => (
          <span className="text-white inline-block my-2">
            {row.original[1]} ({dateToWeek(row.original[1], '周')})
          </span>
        )
      },
      {
        header: () => <span className="text-stock-up">三季度</span>,
        accessorKey: '2',
        enableSorting: false,
        meta: { align: 'center' },
        cell: ({ row }) => (
          <span className="text-white inline-block my-2">
            {row.original[2]} ({dateToWeek(row.original[2], '周')})
          </span>
        )
      },
      {
        header: () => <span className="text-stock-up">四季度</span>,
        accessorKey: '3',
        enableSorting: false,
        meta: { align: 'center' },
        cell: ({ row }) => (
          <span className="text-white inline-block my-2">
            {row.original[3]} ({dateToWeek(row.original[3], '周')})
          </span>
        )
      }
    ],
  ...
  const rateColumns = useMemo<JknTableProps<any>['columns']>(
    () => [
      ...
      {
        header: () => <span className="text-stock-up">前一个交易日</span>,
        accessorKey: '1',
        enableSorting: false,
        meta: { align: 'center' },
        cell: ({ row }) => (
          <span className="inline-block my-2">
            {row.original[1] ? Decimal.create(row.original[1]).mul(100).toFixed(2) : '--'}%
          </span>
        )
      },
      {
        header: () => <span className="text-stock-up">四巫日(当日)</span>,
        accessorKey: '2',
        enableSorting: false,
        meta: { align: 'center' },
        cell: ({ row }) => (
          <span className="inline-block my-2">
            {row.original[2] ? Decimal.create(row.original[2]).mul(100).toFixed(2) : '--'}%
          </span>
        )
      },
      {
        header: () => <span className="text-stock-up">后一个交易日</span>,
        accessorKey: '3',
        enableSorting: false,
        meta: { align: 'center' },
        cell: ({ row }) => (
          <span className="inline-block my-2">
            {row.original[3] ? Decimal.create(row.original[3]).mul(100).toFixed(2) : '--'}%
          </span>
        )
      },
      {
        header: () => <span className="text-stock-up">四巫日(当周)</span>,
        accessorKey: '4',
        enableSorting: false,
        meta: { align: 'center' },
        cell: ({ row }) => (
          <span className="inline-block my-2">
            {row.original[4] ? Decimal.create(row.original[4]).mul(100).toFixed(2) : '--'}%
          </span>
        )
      },
      {
        header: () => <span className="text-stock-up">下一周</span>,
        accessorKey: '5',
        enableSorting: false,
        meta: { align: 'center' },
        cell: ({ row }) => (
          <span className="inline-block my-2">
            {row.original[5] ? Decimal.create(row.original[5]).mul(100).toFixed(2) : '--'}%
          </span>
        )
      }
    ],
  ...
  return (
    <div className="w-[900px] box-border px-8 py-4 mx-auto h-full overflow-y-auto">
      ...
      <div className="text-stock-up mt-12">
        ...
        <div className="mb-12 pt-1">
          <p className="text-center text-lg bg-background m-0 py-3">
            {query.data?.date.year ?? '--'}年巫日前后上涨概率
          </p>
