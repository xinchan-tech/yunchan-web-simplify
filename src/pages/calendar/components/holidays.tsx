...
const Holidays = () => {
  ...
  const queryData = [
    dayjs(active).startOf('year').format('YYYY-MM-DD'),
    dayjs(active).endOf('year').format('YYYY-MM-DD')
  ]
    ...
    () => [
      {
        ...
        align: 'center',
        render: (_, row) => (
          <span style={{ color: getColor(row.date) }} className="text-white inline-block my-4">
            {row.exchange}
          </span>
        )
      },
          ...
          <JknDatePicker onChange={date => date && setActive(date)}>
            {(date, action) => (
              <span className="inline-block w-24" onClick={() => action.open} onKeyDown={() => {}}>
                <CapsuleTabs.Tab disabled label={date ?? '自定义'} value={date ?? 'manual'} />
              </span>
            )}
          </JknDatePicker>
        </CapsuleTabs>
