...
import { CapsuleTabs, JknIcon } from '@/components'
...
export const TimeIndexSelect = () => {
  ...
  const getIsLastDay = useCallback(
    (trading: StockChartInterval) => {
        ...
        <DropdownMenuTrigger asChild>
          <div className="flex-shrink-0">
            <Button reset className="text-xs font-normal">
              {timeIndex.findIndex(v => v === interval) > 3 ? (
                '分时'
              ) : (
                <span className="text-primary">
                  {stockUtils.intervalToStr(interval)}
                  {[
                    StockChartInterval.PRE_MARKET,
                    StockChartInterval.INTRA_DAY,
                    StockChartInterval.AFTER_HOURS
                  ].includes(interval) && getIsLastDay(interval)
                    ? '(上一交易日)'
                    : ''}
                </span>
              )}
        ...
        {timeIndex
          ...
          .map(item => (
            <CapsuleTabs.Tab
              ...
              key={item}
              label={stockUtils.intervalToStr(item)}
              value={item.toString()}
            />
          ))}
      </CapsuleTabs>
      {timeIndex.slice(4).length > showCount ? (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <div className="flex-shrink-0">
              <Button reset className="text-xs font-normal">
                {!timeIndex.slice(4).slice(showCount).includes(interval) ? (
                  '周期'
                ) : (
          ...
          <DropdownMenuContent>
            {timeIndex
              ...
              .slice(showCount)
              .map(item => (
                <DropdownMenuItem key={item} onClick={() => onChangeInterval(+item)}>
                  {stockUtils.intervalToStr(item)}
                </DropdownMenuItem>
              ))}
          </DropdownMenuContent>
        </DropdownMenu>
