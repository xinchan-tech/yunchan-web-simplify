...
export const PeriodSelect = memo(() => {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" alignOffset={-10}>
        <DropdownMenuItem
          data-checked={renderUtils.isTimeIndexChart(interval)}
          onClick={() => chartManage.setInterval(StockChartInterval.INTRA_DAY)}
        >
          分时图
        </DropdownMenuItem>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <span className="cursor-pointer hover:bg-accent rounded flex px-3 h-9">
          <JknIcon.Svg
            name={
              renderUtils.isTimeIndexChart(interval)
                ? 'chart-area'
        ...
        <div className="flex-1 overflow-auto py-2 box-border flex-col flex">
          {!search ? (
            <ToggleGroup className="ml-8 my-2" type="single" value={type} onValueChange={setType as any}>
              <ToggleGroupItem value="main" className="rounded-2xl h-[24px] text-xs px-3 leading-1">
                主图
              </ToggleGroupItem>
              <ToggleGroupItem value="secondary" className="rounded-2xl h-[24px] text-xs px-3 leading-1">
                副图
              </ToggleGroupItem>
            </ToggleGroup>
          ) : null}
            ...
            {indicators.map(i => (
              ...
              >
                {i.collect === 1 ? (
                  <JknIcon.Svg
                    name="fav-star"
                    className="text-[#FFC440] p-1 rounded"
                    size={16}
                    onClick={e => {
                      e.stopPropagation()
                      e.preventDefault()
                      collect.mutate({ id: i.id, collect: false })
                    }}
                  />
                ) : (
                  <JknIcon.Svg
                    ...
                    onClick={e => {
                      e.stopPropagation()
                      e.preventDefault()
                      collect.mutate({ id: i.id, collect: true })
                    }}
                  />
                )}
                <span className="text-foreground">{i.name}</span>
                {!i.authorized ? <JknIcon name="ic_lock" className="rounded-none" /> : null}
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <span className="cursor-pointer hover:bg-accent  h-9 rounded leading-9 px-3 flex items-center justify-center">
          <JknIcon.Svg
            name={viewModeMenuItems.find(i => i.value === viewMode)?.icon as any}
    ...
    <div className="flex h-[500px] flex-col text-sm">
      <div className="border-b-primary flex items-center px-4 flex-shrink-0 ">
        ...
        <Input
          ...
          className="border-none placeholder:text-tertiary text-secondary"
        />
      </div>
        ...
        <div className="flex-1 overflow-auto py-2 box-border">
          {marks.map(i => (
            ...
            >
              ...
              <span
                className={cn(
                  'ml-auto rounded-full w-4 h-4 overlay-mark-check flex items-center justify-center',
                  overlayMark?.mark === i.key ? '!bg-[#089981]' : ''
                )}
              >
                {overlayMark?.mark === i.key ? <JknIcon.Svg name="check" size={10} /> : null}
              </span>
            </div>
          ))}
        </div>
...
const BackTest = () => {
  ...
  return (
    <div
      className={cn(
        'cursor-pointer hover:bg-accent h-full rounded px-3 box-border flex items-center text-sm py-2',
