...
const MainIndicatorSelect = ({ indicators }: { indicators?: Awaited<ReturnType<typeof getStockIndicators>> }) => {
  ...
  return (
    <div className="flex items-center space-x-4">
      ...
      {
        indicators?.main.filter(i => i.name !== '缠论系统').map(item => (
          <HoverCard openDelay={100} closeDelay={100} key={item.id}>
            ...
            <HoverCardContent side="top" className="w-fit p-0">
              <SearchList key={item.id} value={Reflect.ownKeys(mainIndicators).map(v => v.toString())} data={item.indicators.map(v => ({ label: v.name ?? '', value: v.id, extra: v }))} type="multi" name={item.name} onChange={onChangeMainIndicator} />
            </HoverCardContent>
          </HoverCard>
...
const LineTypeSelect = () => {
  ...
  return (
    <div className="flex items-center space-x-3 text-xs">
      <div
        onClick={() => onChangeMainChartType('line')} onKeyDown={() => { }}
        className={cn('flex items-center cursor-pointer', chartType === 'line' && 'text-primary')}><JknIcon name="line_type_1" className="w-4 h-4 mr-1" checked={chartType === 'line'} />折线图</div>
      <div
        onClick={() => onChangeMainChartType('k-line')} onKeyDown={() => { }}
        className={cn('flex items-center cursor-pointer', chartType === 'k-line' && 'text-primary')}><JknIcon name="line_type_2" className="w-4 h-4 mr-1" checked={chartType === 'k-line'} />蜡烛图</div>
    </div>
  )
}
    ...
    <div className="flex items-center space-x-4">
      {
        tabList.data?.map(mark => (
          <HoverCard key={mark.key} openDelay={100} closeDelay={200}>
            ...
            <HoverCardContent side="top" className="w-fit p-0">
              <SearchList search={false} onChange={(v, d) => { kChartUtils.setOverlayMark({ mark: v, type: mark.key, title: d.label }) }} type="single" key={mark.key} data={mark.value.map(t => ({ label: t.name, value: t.key, extra: { title: t.name } }))} name={mark.title} value={overlayMark?.mark} />
            </HoverCardContent>
          </HoverCard>
...
const CoilingList = ({ indicators }: { indicators?: Awaited<ReturnType<typeof getStockIndicators>> }) => {
  ...
  const indicatorItems = (indicators?.main.find(o => o.name === '缠论系统')?.indicators.find(o => o.id === system) as any)?.items as StockIndicator[] | undefined
