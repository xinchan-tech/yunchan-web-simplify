      ...
      <div className="flex-1 order-1 h-full">
        {
          params.type && !['finance'].includes(params.type) ? (
            <KChart onChangeLeftSide={onLeftSideChange} leftSideVisible={leftVisible} onChangeRightSize={onRightSideChange} rightSideVisible={rightVisible} />
          ) : <Finance />
        }
      </div>
      {
        params.type && !['finance'].includes(params.type) && (
          <div id="stock-trading-right-container" data-width={rightVisible} className="order-2 w-[300px] data-[width=hide]:hidden h-full flex-shrink-0">
            <StockInfo />
          </div>
        )
      }
