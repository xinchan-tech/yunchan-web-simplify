    ...
    <div className="h-full bg-muted text-sm">
      <div className="mx-auto h-full w-[800px] relative">
        <div className="absolute border border-solid border-border translate-x-[-100%] -ml-10 top-10">
          <div className="py-4 px-8 cursor-pointer" onClick={() => onScrollTo('系统设置')} onKeyDown={() => { }}> 系统设置</div>
          <div className="py-4 px-8 cursor-pointer" onClick={() => onScrollTo('功能建议')} onKeyDown={() => { }}> 功能建议</div>
          <div className="py-4 px-8 cursor-pointer" onClick={() => onScrollTo('合作联系')} onKeyDown={() => { }}> 合作联系</div>
          <div className="py-4 px-8 cursor-pointer" onClick={() => onScrollTo('版本信息')} onKeyDown={() => { }}> 版本信息</div>
        </div>
        <ScrollArea className="h-full">
          <div data-label="系统设置" className="mt-12 text-lg border-0 border-b border-solid border-dialog-border mb-8 pb-2">系统设置</div>
          <div className="space-y-12">
            <SettingItem label="系统语言">
              <RadioGroup className="flex space-x-8" value={config.language} onValueChange={(value) => config.setLanguage(value as typeof config.language)}>
            ...
            <SettingItem label="涨跌颜色">
              <div className="flex space-x-8">
                ...
                )} onClick={() => config.setSetting({ upOrDownColor: 'upGreenAndDownRed' })} onKeyDown={() => { }}
                >
                  <div className="flex items-center mb-1">绿涨&emsp;<JknIcon name="color_green_up" /></div>
                  <div className="flex items-center">红跌&emsp;<JknIcon name="color_red_down" /></div>
                </div>
                <div className={cn(
                  'py-3 px-6 border border-solid border-dialog-border rounded-sm cursor-pointer',
                  config.setting.upOrDownColor === 'upRedAndDownGreen' && 'border-primary'
                )} onClick={() => config.setSetting({ upOrDownColor: 'upRedAndDownGreen' })} onKeyDown={() => { }}
                >
                  <div className="flex items-center">红涨&emsp;<JknIcon name="color_red_up" /></div>
                  <div className="flex items-center mb-1">绿跌&emsp;<JknIcon name="color_green_down" /></div>
                </div>
              </div>
            </SettingItem>

            <SettingItem label="操作设置">
              <RadioGroup value={config.setting.operation} onValueChange={(value) => config.setSetting({ operation: value as typeof config.setting.operation })}>
                <div className="flex items-center space-x-2">
            ...
            <SettingItem label="缩放设置">
              <RadioGroup className="flex space-x-8" value={config.setting.scale} onValueChange={(value) => config.setSetting({ scale: value as typeof config.setting.scale })}>
                <div className="flex items-center space-x-2">
            ...
            <SettingItem label="报警提示">
              <RadioGroup className="flex space-x-8" value={config.setting.alarmTips} onValueChange={(value) => config.setSetting({ alarmTips: value as typeof config.setting.alarmTips })}>
                <div className="flex items-center space-x-2">
            ...
            <SettingItem label="报警显示">
              <RadioGroup className="flex space-x-8" value={config.setting.alarmShow} onValueChange={(value) => config.setSetting({ alarmShow: value as typeof config.setting.alarmShow })}>
                <div className="flex items-center space-x-2">
            ...
            <SettingItem label="缺口设置">
              <RadioGroup className="flex space-x-8" value={config.setting.gapShow} onValueChange={(value) => config.setSetting({ gapShow: value as typeof config.setting.gapShow })}>
                <div className="flex items-center space-x-2">
          ...
          <div data-label="功能建议" className="mt-12 text-lg border-0 border-b border-solid border-dialog-border mb-8 pb-2">功能建议</div>
          <div className="pr-4">
            <Textarea className="box-border" rows={8} placeholder="请输入建议" />
            <div className="mt-2 text-right">
              <Button>发送</Button>
            </div>
          </div>

          <div data-label="合作联系" className="mt-12 text-lg border-0 border-b border-solid border-dialog-border mb-8 pb-2">
