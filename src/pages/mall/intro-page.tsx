      ...
      <div className="space-y-4 font-normal">
        {
          props.intro.map(i => (
            <div key={i.id}>
              {
                i.title !== ' ' ? (
                  <div className="text-lg p-2 rounded bg-background">{i.title}</div>
                ) : null
              }
                        ...
                        item.auths.map((auth, index) => (
                          ...
                          <div key={index} className="flex items-center text-base">
                            <div className="w-48 text-center">
                              {auth === 'yes' || auth === 'no' ?
                                <JknIcon.Checkbox className="w-3.5 h-3.5" checked={auth === 'yes'} checkedIcon="ic_have" uncheckedIcon="ic_delete" /> :
                                auth}
                            </div>
                            {/* <span>{auth}</span> */}
                          </div>
