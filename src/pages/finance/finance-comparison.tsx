  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i < source.length; i++) {
      ...
      r.push({
        ...
        data: data.map(v => [
          v.fiscal_year.slice(2) + (params.period === 'quarter' ? ` ${v.fiscal_period}` : ''),
          v[params.type as keyof (typeof data)[0]] as unknown as number
        ])
      })
    }
      ...
      <div className="flex items-center px-4 box-border py-2">
        ...
        <ToggleGroup
          ...
          onValueChange={(v: string) =>
            setParams(draft => {
              draft.type = v as string
            })
          }
        >
          <ToggleGroupItem className="w-24 h-8" value="revenues">
            总营收
          </ToggleGroupItem>
          <ToggleGroupItem className="w-24 h-8" value="net_income_loss">
            净利润
          </ToggleGroupItem>
          <ToggleGroupItem className="w-24 h-8" value="net_cash_flow_from_operating_activities">
            现金流
          </ToggleGroupItem>
          <ToggleGroupItem className="w-24 h-8" value="market_cap">
            总资产
          </ToggleGroupItem>
          <ToggleGroupItem className="w-24 h-8" value="liabilities_rate">
            负债率
          </ToggleGroupItem>
        </ToggleGroup>
      </div>
      <div className="flex items-center px-4 box-border py-2">
        ...
        <div>
          {params.symbols.map((symbol, index) => (
            <div
              key={symbol}
              className="inline-block min-w-16 mr-3 px-2 py-1 bg-accent text-white rounded-sm text-center relative"
              style={{ backgroundColor: colorUtil.colorPalette[index] }}
            >
              <span
                className="absolute -right-2 -top-2 cursor-pointer w-4 h-4 text-xs bg-gray-600 rounded-full text-center"
                onClick={() =>
                  setParams(d => {
                    d.symbols.splice(index, 1)
                  })
                }
                onKeyDown={() => {}}
              >
