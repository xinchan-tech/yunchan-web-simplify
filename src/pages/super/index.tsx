...
const SuperStock = () => {
  ...
  return (
    <SuperStockContext.Provider
      value={{ data: category as unknown as SuperStockContext['data'], register, unregister }}
    >
      {result.hasResult ? (
        ...
        <ScrollArea className="bg-muted h-full">
          <div className="w-[1366px] mx-auto ">
            <div className="p-12 box-border ">
              ...
              {type === SuperStockType.Super && <div className="mt-8 bg-accent py-1 text-center text-sm">技术面</div>}
              <div>
                <FirstStep />
              </div>
              ...
              {type === SuperStockType.Super && <div className="mt-8 bg-accent py-1 text-center text-sm">基本面</div>}
...
const SuperStockTypeTab = (props: SuperStockTypeTabProps) => {
  ...
  return (
    <div className="super-stock-type">
      <div
        className={cn(props.type === SuperStockType.Tech && 'super-stock-type-active')}
        onClick={() => _onClick(SuperStockType.Tech)}
        onKeyDown={() => {}}
      >
        技术面
      </div>
      <div
        className={cn(props.type === SuperStockType.Basic && 'super-stock-type-active')}
        onClick={() => _onClick(SuperStockType.Basic)}
        onKeyDown={() => {}}
      >
        基本面
      </div>
      <div
        className={cn(props.type === SuperStockType.Super && 'super-stock-type-active')}
        onClick={() => _onClick(SuperStockType.Super)}
        onKeyDown={() => {}}
      >
        超级组合
      </div>
