  ...
  const columns = useMemo<JknTableProps<PlateStockTableDataType>['columns']>(
    () => [
      ...
      },
      {
        ...
        accessorKey: 'name',
        meta: { align: 'left' },
        cell: ({ row }) => <StockView name={row.getValue('name')} code={row.original.code as string} />
      },
      {
        header: '现价',
        accessorKey: 'price',
        ...
        cell: ({ row }) => (
          <NumSpan
            value={numToFixed(row.getValue<number>('price')) ?? 0}
            isPositive={row.getValue<number>('percent') >= 0}
          />
        )
      },
      {
        ...
        cell: ({ row }) => (
          <NumSpan
            ...
            block
            decimal={2}
            value={row.getValue<number>('percent') * 100}
            isPositive={row.getValue<number>('percent') >= 0}
            symbol
          />
        )
