 ...
 * @returns 经济数据走马灯组件
 */
export const SuperCarousel: React.FC<
  React.HTMLAttributes<HTMLDivElement> & SuperCarouselProps
> = ({ 
  items, 
  opts, 
  setApi, 
 ...
 * @returns 经济数据项组件
 */
const EconomicDataItem: React.FC<EconomicDataItemProps> = ({ 
  item, 
  onCheckChange,
  isChecked = false
}) => {
      ...
      >
        ...
        <div className="flex flex-row items-center justify-between">
          ...
          <div className="flex flex-col items-end">
            ...
            <div className='text-base' style={{ color: item.percent > 0 ? '#089981' : '#F23645' }}>{item.percent.toFixed(2)}%</div>
          </div>
        </div>
      </div>
    </div>
