import { type StockExtend, addStockCollect, getStockCollectCates, getStockCollects, removeStockCollect, removeStockCollectCate, updateStockCollectCate } from "@/api"
import { AddCollect, AiAlarm, Button, CollectCapsuleTabs, Input, JknAlert, JknCheckbox, JknIcon, JknRcTable, type JknRcTableProps, NumSpanSubscribe, Popover, PopoverAnchor, PopoverClose, PopoverContent, PopoverTrigger, StockView, useModal } from "@/components"
...
const GoldenPool = () => {
  ...
  const columns: JknRcTableProps<TableDataType>['columns'] = useMemo(() => [
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="cursor-pointer text-tertiary text-center w-full" onClick={() => onRemove(row.symbol, row.symbol)} onKeyDown={() => { }}>
          <JknIcon name="del" className="w-4 h-4" />
        </div>
      )
    },
    {
      title: <GoldenPoolBatch activeStock={activeStock} checked={checked} onCheckedChange={e => setCheckedAll(e ? list.map(item => item.symbol) : [])}
        onUpdate={() => {
          collects.refetch()
          setCheckedAll([])
        }}
...
const GoldenPoolTable = () => {
  ...
  const columns: JknRcTableProps['columns'] = [
    ...
    { title: '创建时间', dataIndex: 'create_time', align: 'left', render: (v) => v !== '0' ? dayjs(+v * 1000).format('YYYY-MM-DD') : '-' },
    {
      title: '操作',
      dataIndex: 'opt',
      align: 'center',
    ...
    <Popover>
      ...
      <PopoverContent className="w-52 text-center">
        ...
        <div className="px-4 mt-4">
          <Input size="sm" className="border-transparent border-b-gray-500 rounded-none" placeholder="输入金池名称" onChange={e => setName(e.target.value)} />
...
const GoldenPoolBatch = (props: GoldenPoolBatchProps) => {
  ...
  const onRemoveBatch = async (cateId: string) => {
    const [err] = await to(removeStockCollect(
      {
      ...
      <PopoverContent align="start" side="left" className="w-56">
        <div className="rounded pb-2 relative">
          ...
          <div className="flex items-center justify-between px-4 py-2">
            ...
            <Button size="mini" variant="destructive" onClick={() => onAction('remove', props.activeStock)}>删除</Button>
          </div>
          {
            cates.data?.filter(cate => +cate.id !== +props.activeStock).map(cate => (
              <div key={cate.id} className="flex items-center justify-between px-4 py-2">
                <span>{cate.name}</span>
                <Button size="mini" onClick={() => onAction('add', cate.id)}>添加</Button>
              </div>
            ))
          }
              ...
              <div className="absolute bottom-0 left-0 top-0 right-0 bg-background/60">
                <div className="text-center flex flex-col justify-center h-full space-y-4">
                  ...
                  <div className="flex justify-center space-x-4">
                    <Button size="mini" onClick={() => action.current?.cateId && (action.current?.type === 'add' ? onAdd(action.current?.cateId) : onRemoveBatch(action.current?.cateId))}>确定</Button>
