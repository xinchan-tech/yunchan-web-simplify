import { type CreateGroupRecord, getCreateGroupHistoryService } from '@/api/group-chat'
...
const CreateHistory = (props: {
    ...
    content: (
      <div>
        <div className="flex p-4 h-[200px] items-center justify-center">
          {curRecord?.status === '2' && `申请被驳回, 驳回原因: ${curRecord?.reject_reason}`}
  ...
  const columns: JknTableProps<CreateGroupRecord>['columns'] = [
    ...
    },
    {
      ...
      accessorKey: 'name',
      enableSorting: false,
      cell: ({ row }) => <span className="block py-1">{row.original.name || ''}</span>,
    ...
    },
    {
      ...
      accessorKey: 'price',
      enableSorting: false,
      cell: ({ row }) => <span className="block py-1">${row.original.price || '0'}</span>,
      meta: { align: 'center' }
    },
    {
      header: '人数上限',
      accessorKey: 'max_num',
      enableSorting: false,
      cell: ({ row }) => <span className="block py-1">{row.original.max_num || '0'}</span>,
      meta: { align: 'center' }
    },
    {
      header: '审核状态',
      ...
      enableSorting: false,
      cell: ({ row }) => (
        <span className="block py-1" style={{ color: statusData[row.original.status].color }}>
        ...
        return (
          <>
            ...
            {row.original.status === '2' && (
              <span
                onClick={() => {
                  typeof props.onReCreate === 'function' && props.onReCreate(row.original)
                }}
