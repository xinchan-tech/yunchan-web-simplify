import { type CreateGroupRecord, applyCreateGroupService, type createGroupRequest } from '@/api'
import {
  Button,
  Checkbox,
  Form,
...
const memberLimitConf = ['50', '100', '200', '500', '1000', '2000', '3000', '4000', '5000']

const priceConf = ['0.0', '9.9', '19.9', '29.9', '39.9', '49.9', '59.9', '69.9', '79.9', '89.9', '99.9', '990']
    ...
    content: (
      <div className="pl-[50px] pr-[50px] pt-4 pb-10">
        <ScrollArea className="h-[450px] agreement-scroll-box p-4">
          ...
          嘉可能 不保证任何被告知/报告的问题将由 嘉可能 解决，即使 嘉可能选择为了解决某个问题提供信息，也是如此。
          <br /> <br />
          第三方链接 <br /> <br />
          ...
          儿童
          <br /> <br />
          我们不会在明知对方为年龄不满 13 周岁的儿童的情况下收集此人的个人信息。如果我们发现我们收集了年龄不满 13
          周岁的儿童的个人信息，我们将采取措施尽快地删除此等信息。
          <br /> <br />
          信息的存储和交换 <br /> <br />
        ...
        <form onSubmit={form.handleSubmit(createGroupMutation.mutate as any)}>
          <div className="flex">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-1 items-center ">
                  <FormLabel className="w-[90px] text-right mr-2">社群名称：</FormLabel>
                  <FormControl className={'border-dialog-border rounded-sm  bg-accent inline-block'}>
                    <Input
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-1 items-center ">
                  <FormLabel className="w-[90px] text-right mr-2">社群头像：</FormLabel>
                  <FormControl>
                    <AliyunOssUploader value={field.value} onChange={field.onChange}></AliyunOssUploader>
                  </FormControl>
                </FormItem>
              )}
            />
          ...
          <div className="flex mt-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-1 items-center">
                  <FormLabel className="w-[90px] text-right mr-2">社群标签：</FormLabel>
                  <FormControl className={'border-dialog-border rounded-sm  bg-accent inline-block'}>
                    <Input
                      className="border-none placeholder:text-tertiary flex-1"
                      style={{ marginTop: '0' }}
          ...
          <div className="flex mt-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-1 items-center">
                  <FormLabel className="w-[90px] text-right mr-2">付费等级：</FormLabel>
                  <FormControl className={'border-dialog-border rounded-sm  bg-accent inline-block'}>
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger style={{ marginTop: '0' }} className="flex-1 bg-accent">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-1 items-center">
                  <FormLabel className="w-[90px] text-right mr-2">会员上限：</FormLabel>
                  <FormControl className={'border-dialog-border rounded-sm  bg-accent inline-block'}>
                    <Select onValueChange={field.onChange} value={field.value}>
                      <SelectTrigger style={{ marginTop: '0' }} className="flex-1 bg-accent">
                        <SelectValue />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="mt-2 flex items-center">
                      <FormControl className={'border-dialog-border rounded-sm mt- bg-accent inline-block'}>
                        <Select onValueChange={field.onChange} value={field.value}>
                          <SelectTrigger style={{ marginTop: '0' }} className="flex-1 bg-accent">
                            <SelectValue />
                          </SelectTrigger>
          ...
          <div className="flex mt-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-1 items-center">
                  <FormLabel className="w-[90px] text-right mr-2">社群简介：</FormLabel>
                  <FormControl className={'border-dialog-border rounded-sm  bg-accent inline-block'}>
                    <Textarea
                      className="border-none placeholder:text-tertiary flex-1"
          ...
          <div className="flex mt-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-1 items-center">
                  <FormLabel className="w-[90px] text-right mr-2">社群公告：</FormLabel>
                  <FormControl className={'border-dialog-border rounded-sm  bg-accent inline-block'}>
                    <Textarea
                      className="border-none placeholder:text-tertiary flex-1"
                      style={{ marginTop: '0', resize: 'none' }}
