...
const GroupChatMsgList = forwardRef((props, ref) => {
  ...
  const judgeNotOver = () => {
    if (scrollDomRef.current) {
      return (
        scrollDomRef.current.scrollHeight <= scrollDomRef.current.offsetHeight
      );
    }
    return true;
  };
  ...
  messageListener = (msg) => {
    ...
    if (
      messagesRef.current.findIndex((m) => m.clientMsgNo === msg.clientMsgNo) <
      0
    ) {
