import { Message , MessageText, MessageContentType} from "wukongimjssdk";
  ...
  private parseMention(): Array<Part> {
    ...
    const mentionMatchResult = text.match(/@([\S])+/m);
      ...
      parts.push(
        new Part(
          PartType.mention,
          text.substr(index, mentionMatchResult[0].length),
          data
        )
      );
    ...
    for (let index = 0; index < len; index++) {
      ...
      if (part.type === PartType.text) {
        ...
        while (text.length > 0) {
          ...
          newParts.push(
            new Part(PartType.link, text.substr(index, matchResult[0].length))
          );
          text = text.substring(index + matchResult[0].length);
        }
      } else {
