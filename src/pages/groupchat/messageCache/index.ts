...
class LocalCacheManager {
  ...
  private maintainMemoryCache(groupId: string): void {
    ...
    const filtered = this.liveMessagesCache.get(groupId)?.filter(m => m.timestamp > cutoff) || []
  ...
  private async queryFromDB(
    ...
    return new Promise((resolve, reject) => {
      ...
      // 暂时只有往前面查
      let count = 0
      const range = IDBKeyRange.bound([groupId, left], [groupId, right])
      // 查最近的 limit数量的信息
      ...
      if (isLatest === true) {
        const newRange = IDBKeyRange.bound([groupId, 0], [groupId, Number.POSITIVE_INFINITY])
...
class SyncManager {
  ...
  public async syncMessages(groupId: string, messages: any[]): Promise<void> {
    ...
    await Promise.all(messages.map(msg => this.cacheManager.cacheMessage(msg)))
