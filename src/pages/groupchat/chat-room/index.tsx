...
export const ChatRoom = () => {
  ...
  const channelQuery = useQuery({
    ...
    queryFn: async () =>
      WKSDK.shared()
        ...
        .then(() => WKSDK.shared().channelManager.getChannelInfo(channel!)),
  ...
  const noticeModal = useModal({
    content: (
      <ChatRoomNotice
        notice={channelDetail?.notice ?? ''}
        onConfirm={() => {
          readChannelNotice(channel!.channelID).then(() => {
...
const ChatRoomNotice = ({ notice, onConfirm }: { notice: string; onConfirm: () => void }) => {
  ...
  return (
    <div className="chat-room-notice py-6 px-5">
      ...
      <div className="chat-room-notice-content text-xs text-tertiary leading-5 h-[90px] overflow-y-auto">{notice}</div>
