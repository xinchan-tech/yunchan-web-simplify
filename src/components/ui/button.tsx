  ...
  'inline-flex border items-center font-normal justify-center gap-2 border-solid cursor-pointer whitespace-nowrap rounded-sm text-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        outline:
          'border-border bg-transparent text-foreground hover:bg-accent hover:text-foreground hover:border-input',
        secondary: 'bg-secondary text-secondary-foreground shadow-sm hover:bg-secondary/80',
        ghost: 'hover:bg-accent hover:text-accent-foreground',
        link: 'text-primary underline-offset-4 hover:underline',
        icon: 'bg-transparent !p-0 m-0 border-0 !h-auto w-auto',
        text: 'bg-transparent text-primary-foreground border-transparent'
      },
      size: {
        default: 'h-9 px-1 py-2 rounded-[6px]',
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading, children, block, reset, ...props }, ref) => {
    ...
    return (
      <Comp className={cn(buttonVariants({ variant, size, loading, block, reset, className }))} ref={ref} {...props}>
