  ...
  useEffect(() => {
    if (token) {
      ...
      const unSubscribe = ws.onAlarm(e => {
        const n = toast(<AlarmContent data={e} onClose={() => { toast.dismiss(n) }} />, {
          duration: 10 * 1000
        })
      })
    ...
    <div className="flex leading-none font-normal w-full cursor-pointer" onClick={() => onClick()} onKeyDown={() => { }}>
      ...
      <div className="px-2.5 py-2.5 w-[360px] space-y-1">
        <div className="flex items-center w-full">
          ...
          <JknIcon.Svg name="close" size={12} hoverable onClick={e => { e.stopPropagation(); onClose() }} className="ml-auto p-2" />
        </div>
        <div className="flex items-center text-sm space-x-2">
          {
            data.type === AlarmType.AI ? (
              <>
                <span>
                  {stockUtils.intervalToStr(data.stock_cycle)}
                </span>
        ...
        {
          data.type === AlarmType.AI ? (
            <Star.Rect total={5} count={data.score_total} className="ml-0.5 w-1.5 h-2.5" activeColor={data.bull === '1' ? useConfig.getState().getStockColor(true, 'hex') : useConfig.getState().getStockColor(false, 'hex')} />
