...
type TimeshareChartType = 'pre' | 'post' | 'after' | 'full-day'
...
interface JknChartIns {
   ...
   * 设置分时图
   */
  setTimeShareChart: (type: TimeshareChartType) => void
  /**
   * 恢复正常图表
   */
  restoreNormalChart: () => void
...
export const JknChart = forwardRef<JknChartIns, JknChartProps>((props: JknChartProps, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    setTimeShareChart: (chartType) => {
      // if (isTimeShare.current) return
      const { up: upColor, down: downColor } = getStockColor()
      const splitId = 'split-indicator-large-cap'
      const ticks = stockUtils.getIndexTimeTick()
      ...
      if (chartType === 'pre') {
        ...
        ticks.after = 0
        ticks.pre = 0
      } else if (chartType === 'after') {
        ticks.pre = 0
        ticks.post = 0
      }
      ticks.total = ticks.pre + ticks.post + ticks.after
      isTimeShare.current = true
      chart.current?.setLeftMinVisibleBarCount(1)
      chart.current?.setXAxisTick(ticks.total)
      ...
      chart.current?.createIndicator(
        {
          ...
          calcParams: [
            [
              ticks.pre / (ticks.total),
              (ticks.pre + ticks.post) / (ticks.total)
            ]
          ]
        },
        true,
      ...
      chart.current?.setStyles({
        candle: {
          ...
          area: {
            lineColor: data => {
              const postData = data.slice(0, ticks.post + ticks.pre).pop()
              const lastData = data[data.length - 1]

              const lastColor = Decimal.create(lastData?.close).gt(lastData?.prevClose ?? 0) ? upColor : downColor
              const preColor = data.length > ticks.pre ? '#50535E' : lastColor

              const afterColor = data.length >= ticks.total ? '#50535E' : lastColor

              return [
                { type: 'segment', color: preColor, offset: ticks.pre },
                {
                  type: 'segment',
                  color: Decimal.create(postData?.close).gt(postData?.prevClose ?? 0) ? upColor : downColor,
                  offset: ticks.post + ticks.pre
                },
            ...
            backgroundColor(data) {
              ...
              const preColor = data.length > ticks.pre ? '#DBDBDB' : lastColor
              const afterColor = data.length >= ticks.total ? '#DBDBDB' : lastColor
    ...
    restoreNormalChart: () => {
      console.log(isTimeShare.current)
      // if (!isTimeShare.current) return
      const splitId = 'split-indicator-large-cap'
      chart.current?.removeIndicator({
        id: splitId
