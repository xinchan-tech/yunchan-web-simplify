...
const StockSelect = (props: StockSelectProps) => {
      ...
      <Popover modal open={open} onOpenChange={v => !v && setFalse()}>
        <PopoverAnchor asChild>
          <div className="border border-solid border-dialog-border rounded-sm flex items-center px-2">
            ...
            <Input value={keyword} onChange={e => setKeyword(e.target.value)} className="border-none" size="sm" onClick={() => setTrue()} placeholder="请输入股票代码" />
          </div>
        </PopoverAnchor>
              ...
              <div>
                ...
                <ScrollArea className="h-[300px]">
                  {stockList.history.map((ele) => (
                    <div
                      className="h-[49px] border-0 border-b border-solid border-border flex items-center hover:bg-accent cursor-pointer"
                      key={ele[1]}
                      onClick={() => _onClick(ele[1])}
                      onKeyDown={() => { }}
                    >
                      <div className="flex-shrink-0"><img src={ele[0]} alt="" /></div>
      ...
      <div ref={wrapperRef}>
        {list.map((ele) => (
          <div
            ...
            onKeyDown={() => { }}
          >
            <div className="flex-shrink-0"><img src={ele.data[0]} alt="" /></div>
            <div className="flex-1 overflow-hidden">
