...
function svgFind(directoryPath: string, idPrefix: string): string[] {
  ...
  for (const dirent of directs) {
    if (dirent.isDirectory()) {
      ...
      let svgContent = readFileSync(join(directoryPath, dirent.name), 'utf-8')
        .replace(clearReturn, '')
        .replace(svgTitle, ($1, $2: string) => {
          let width = 0
          let height = 0
          let content = $2.replace(clearHeightWidth, (match: string, prop: string, value: number) => {
          ...
          const name = idPrefix.replace('[name]', dirent.name.replace('.svg', ''))
...
export const pluginSvgSpriteIcons = (options: SvgSpriteLoaderOptions): RsbuildPlugin => ({
  name: 'plugin-svg-sprite-icons',
