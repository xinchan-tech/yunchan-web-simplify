...
class SearchField<T> extends StatefulWidget {
  ...
  /// search text.
  final Future<List<SearchFieldListItem<T>>>? Function(String)? onSearchTextChanged;
  ...
  void initialize() {
    ...
    _focus!.addListener(() {
      ...
      if (isSuggestionExpanded) {
        ...
        if (widget.initialValue == null && widget.initWidget == null) {
      ...
      builder: (BuildContext context, AsyncSnapshot<List<SearchFieldListItem<T>?>?> snapshot) {
        if (snapshot.data == null || !isSuggestionExpanded) {
              ...
              child: Container(
                ...
                decoration: widget.suggestionItemDecoration?.copyWith(
                      border: widget.suggestionItemDecoration?.border ??
                          Border(
                            bottom: BorderSide(color: widget.marginColor ?? onSurfaceColor.withOpacity(0.1), width: 1),
                          ),
                    ) ??
                    BoxDecoration(
                      border: index == snapshot.data!.length - 1
                          ...
                          : Border(
                              bottom:
                                  BorderSide(color: widget.marginColor ?? onSurfaceColor.withOpacity(0.1), width: 1),
                            ),
                    ),
          ...
          return AnimatedContainer(
            duration:
                widget.suggestionDirection == SuggestionDirection.up ? Duration.zero : Duration(milliseconds: 300),
            height: _totalHeight,
            alignment: Alignment.centerLeft,
    ...
    return OverlayEntry(
        builder: (context) => StreamBuilder<List<SearchFieldListItem?>?>(
            ...
            builder: (BuildContext context, AsyncSnapshot<List<SearchFieldListItem?>?> snapshot) {
              ...
              return Positioned(
                left: offset.dx,
                width: widget.suggestionWidth,
                child: CompositedTransformFollower(
                    offset: widget.offset ?? yOffset, link: _layerLink, child: Material(child: _suggestionsBuilder())),
              );
            }));
  }
      ...
      child: TextFormField(
        ...
        onChanged: (query) async {
          ...
          if (widget.onSearchTextChanged != null) {
            searchResult = await widget.onSearchTextChanged!(query) ?? [];
          } else {
            if (query.isEmpty) {
              _createOverlay();
              if (widget.initWidget == null) {
            ...
            for (final suggestion in widget.suggestions) {
              ...
              } else if (suggestion.searchKey.toLowerCase().startsWith(query.toLowerCase())) {
                searchResult.add(suggestion);
              }
            }
          }
